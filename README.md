...
個人用の小道具だが、公開URLで動いている以上の整理（2026-09-16 自己監査）。

**持っていないもの＝事故の目がないもの**
- 依存パッケージがゼロ（サプライチェーン経由の混入なし）
- APIキー・トークン・環境変数を一切使っていない。DBもログインも個人情報も無い（盗まれるデータが存在しない）
- `.gitignore` で `.env` と `.vercel` を除外済み

**気をつけている点**
- 入力（劇場スラッグ・日付・作品ID・時刻）は全部正規表現で検証。ドットを許可していないのでパス遡りは不可
- 取得先・転送先のドメインはすべてコード側にハードコード。外部から任意のURLを取得・転送させられない（SSRF／オープンリダイレクト対策）
- **映画館サイト由来の文字列（作品名・劇場名・スクリーン名）は必ずエスケープしてから表示する。**
  向こうのデータに `<script>` が混ざったらこちらのページで動くため
- 画面には CSP を入れて、外部スクリプトの読み込みと外部への通信を禁止している

**残っているリスク（承知のうえ）**
- 誰でも叩ける公開エンドポイントなので、大量アクセスされると Vercel の使用量を食い、映画館サイトへの中継アクセスも増える。
  URLを広めない運用でしのいでいる。気になるなら Vercel 側の使用量上限を設定する
- 転送先は映画館の公式サイトそのもの。向こうが改ざんされた場合はこちらも巻き込まれる（構造上避けられない）
