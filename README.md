...
| 109シネマズ | `c109-` | 20館 | ✕（販売開始後に購入リンクが出る） |
| T・ジョイ（新宿バルト9・横浜 ほか） | `tj-` | 20館 | ✕（掲載＝販売開始。当日＋2日先まで） |
| TOHOシネマズ | `toho-` | 72館 | △（識別子は先に取れるが、購入がPOSTなので1タップ挟まる） |

合計296館。実戦投入済みはユナイテッド・シネマのみで、他は実データでのテストのみ。
...
- 劇場IDは全20館ぶん取得済み（T・ジョイ横浜=190、横浜ブルク13=170、こうのすシネマ=900 など）。トップページの劇場リンクはURLの前後に空白が入っているので、抽出時に注意
...
画面は**外部フォント・画像・ライブラリを一切使わない単一HTML**（約17KB）。
販売開始の瞬間に描画で待たされたら本末転倒なので、影やぼかしも最小限にしてある。
劇場一覧は起動時に1回だけ取得し、系列の絞り込みも地名検索もブラウザ内で完結する（通信は増えない）。

```
https://<your-app>.vercel.app/
```

**系列 → 劇場**の2段で選ぶ（296館を1つのプルダウンに並べるとスクロールが地獄になるため）。
劇場は**地名でも検索できる**（「横浜」と入れると系列をまたいで T・ジョイ横浜／横浜ブルク13／TOHOららぽーと横浜 が出る）。
選んだ系列と劇場は記憶される。日付 → 作品 → 上映回、とタップで辿れる。
