...
export function scheduleDataLoader(config) {
  return async function load(th, d) {
    ...
    nos.forEach((no, i) => {
      ...
      events[no] = evs;
      films.push({ film: no, name: evs[0].name?.ja?.trim() || `作品${no}`, first: evs[0].startDate });
    });
    // 公式サイトの索引順だと「メンバーズカード受付」のような入会受付枠が先頭に来ることがある。
    // その日の最初の上映時刻が早い順に並べ替える（受付枠は深夜の時間帯なので後ろに回る）
    films.sort((a, b) => String(a.first).localeCompare(String(b.first)));
    films.forEach((f) => delete f.first);
