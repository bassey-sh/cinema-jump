...
export async function loadAeon(th, d) {
  ...
  for (const [film, evs] of Object.entries(day)) {
    ...
    films.push({ film, name: evs[0].name?.ja?.trim() || `作品${film}`, first: evs[0].startDate });
  }
  // 最初の上映時刻が早い順（チェーンをまたいで並びの感覚をそろえる）
  films.sort((x, y) => String(x.first).localeCompare(String(y.first)));
