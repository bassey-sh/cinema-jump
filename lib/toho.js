...
// G は「まだ売っていない」と「もう売り終わった」の両方に使われるので、上映開始時刻で振り分ける
const STATUS = {
  A: ['onsale', '○'],
  B: ['onsale', '○'],
  C: ['onsale', '△'],
...
export async function loadToho(th, d) {
  ...
  const screeningsOf = (film) => {
    ...
    const nowJst = new Date(Date.now() + 9 * 3600e3).toISOString().slice(0, 16); // YYYY-MM-DDTHH:MM
    return (m?.list || []).map((ev) => {
      const start = `${d}T${pad(ev.showingStart || '')}`;
      const [status, seat] = STATUS[ev.unsoldSeatInfo?.unsoldSeatStatus]
        // G（販売期間外）は上映前なら「販売前」、上映が始まっていれば「販売終了」
        || [start > nowJst ? 'before' : 'closed', null];
      const scr = ev.screen || {};
      return {
